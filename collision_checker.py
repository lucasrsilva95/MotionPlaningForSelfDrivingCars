        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                for c in range(len(self._circle_offsets)):
                    circle_x = path[0][j] + self._circle_offsets[c] * cos(path[2][j])
                    circle_y = path[1][j] + self._circle_offsets[c] * sin(path[2][j])
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            x_diff = paths[i][0][-1] - paths[j][0][-1]
                            y_diff = paths[i][1][-1] - paths[j][1][-1]
